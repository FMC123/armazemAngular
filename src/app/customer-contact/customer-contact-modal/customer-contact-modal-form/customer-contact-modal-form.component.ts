...
@Component({
  selector: 'app-customer-contact-modal-form',
  templateUrl: './customer-contact-modal-form.component.html'
})

export class CustomerContactModalFormComponent implements OnInit {
  @Input() id: string;
  @Input() person: Person;
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      this.customerContact = this.personModalService.findCustomerContact(this.id);
    }else {
  ...
  save() {
    ...
    this.personModalService.saveCustomerContact(this.customerContact);
